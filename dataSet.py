...
from trainTest.read_data import read_file
...
class DataSet(object):
    def __init__(self,path):
        ...
        self.Y_train = None
        self.Y_test = None
        print("DataSet_extract_date_before：")
        self.extract_data(path)
        print("DataSet_extract_date_after：")
    ...
    def extract_data(self,path):
        ...
        #重新格式化和标准化
        # 基于tensorflow
        X_train = X_train.reshape(X_train.shape[0], 1,241, 132)
        X_test = X_test.reshape(X_test.shape[0], 1,241, 132)
        print("X_train维度2", X_train.shape)
        
        X_train = X_train.astype('float32')/255
        X_test = X_test.astype('float32')/255
        print("X_train维度3", X_train.shape)
        print("X_train[1]",X_train[1])
