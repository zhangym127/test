# coding= utf-8
from trainTest.dataSet import DataSet
...
class Model(object):
    ...
    #建立一个CNN模型，一层卷积、层卷积、一层一层池化、一池化、抹平之后进行全链接、最后进行分类      其中flatten是将多维输入一维化的函数 dense是全连接层
...
#         plot_model(model,to_file='G:/desktop/myProject/model.png')
    ...
    def train_model(self):
        ...
        self.model.fit(self.dataset.X_train,self.dataset.Y_train,epochs=12,batch_size=20,callbacks=[TensorBoard(log_dir='log')])#TensorBoard
    ...
    def predict(self,img):
        img = img.reshape((1, 1,241, 132))#出错则更改为img = img.reshape((1,  174, 212,1))
